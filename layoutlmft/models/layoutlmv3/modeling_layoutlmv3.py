...
class LayoutLMv3ForTokenClassification(LayoutLMv3PreTrainedModel):
    ...
    def forward(
        ...
        if self.heuristic == 'nearest':
            ...
            gat_output = torch.cat([gat_output, sequence_output[:, adjs_feat_shape:, :]], dim=1)
            ...
            for adj in adjs:
                gat_output = sequence_output[:, :adjs_feat_shape, :]
                gat_output = self.GAT(gat_output, adj, attention_mask[:,:adjs_feat_shape])
                gat_output = self.dropout(gat_output)
                gat_outputs.append(gat_output)
            gat_outputs = torch.stack(gat_outputs)
            gat_output = torch.mean(gat_outputs, dim=0)
            gat_output = torch.cat([gat_output, sequence_output[:, adjs_feat_shape:, :]], dim=1)
            logits = self.classifier(gat_output)
...
class LayoutLMv3ForTokenClassificationGAT(LayoutLMv3PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
