...
do
 for sd in 0
  do
   ...
   --output_dir results/test-layoutlmv3-cord-gat-$heuristic-$sz-$sd --segment_level_layout 1 --visual_embed 1 --input_size 224 \
  ...
  --evaluation_strategy steps --eval_steps 500 --logging_steps 100 --logging_first_step --logging_strategy steps --max_steps 1000  \
   --per_device_train_batch_size 2 --dataloader_num_workers 8 --warmup_ratio 0.1 --gradient_accumulation_steps 1 --fp16 --sz-$sz \
    --sd-$sd --hd-$hd --heuristic-$heuristic --deg-$deg $path/data/path_config_cord.json
