...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        sz, sd = rem_args[0].split('-')[-1], rem_args[1].split('-')[-1]
        path_config = json.load(open(rem_args[-1],'r'))
        # pickle_path = '/home/prashant/unilm/layoutlmv3/data/funsd-aug2'
        # # pickle_path = os.path.join(rem_args[2], 'data-sz{}-sd{}'.format(sz,sd))
        if len(rem_args) > 2:
    ...
    def update_config(config, gat_config_path = "data/gat_params.json"):
        """
        Update the configuration dictionary with parameters specific to GAT (Graph Attention Network) model.

        Parameters:
            config (dict): The config dictionary to be updated.
            gat_config_path (str): Path of the GAT parameters config

        Returns:
            dict: The updated config.

        Notes:
            The function reads GAT configuration parameters from a JSON file located at gat_config_path
            It updates the provided configuration dictionary with these parameters and additional ones:
            'nheads_gat' is set based on the provided number of heads (nheads).
            'nhid_gat' is calculated as 768 divided by the number of heads.
            'device' is set to 'cuda:0'. (can be changed according to your GPU settings)
            'heuristic' is set to the provided heuristic parameter.
        """
        # nfeat, nhid, nclass, dropout, alpha, nheads
        gat_config_params = json.load(open(gat_config_path,'r'))
        gat_config_params['nheads_gat'] = nheads
        gat_config_params['nhid_gat'] = 768//nheads
        # device = training_args.device
        gat_config_params.update({'device':'cuda:0', 'heuristic':heuristic})
        config.update(gat_config_params)
        return config
        ...
        if heuristic == 'angles':
            ...
            try:
                ...
                if type1=='eval' or type1=='test':
                    pickle.dump(all_adjs, open(os.path.join(adj_save_path,f'cord_{type1}_adjs_{theta}_angles_v2_full.pkl'),'wb'))
    ...
    if training_args.do_eval:
        ...
        eval_dataset = eval_dataset.map(
            ...
            remove_columns=remove_columns,
            num_proc=data_args.preprocessing_num_workers,
            load_from_cache_file= data_args.overwrite_cache, fn_kwargs = {'type1':'eval'},
        )
